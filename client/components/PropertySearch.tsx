...
export default function PropertySearch() {
  ...
  const fetchProperties = useCallback(async (reset: boolean) => {
    if (reset) setLoading(true);
    else setLoadingMore(true);

    try {
      const query = buildQuery(reset ? undefined : nextCursor ?? undefined);
  ...
  <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-4 mb-6 grid grid-cols-2 md:grid-cols-5 gap-3">
    ...
    >
      ...
      {propertyTypes.map((type) => (
        <option key={type.id} value={type.id}>{type.property_type}</option>
      ))}
    </select>
  ...
  {hasMore && (
    <div className="flex justify-center mt-8">
      <button
        ...
        className="bg-green-600 text-white px-6 py-2.5 rounded-lg font-medium hover:bg-green-700 disabled:opacity-60 transition-colors"
      >
