...
export const getSimilarProperties = async (req: Request, res: Response) => {
  try {
    ...
    const current = await pool.query(
      `SELECT city, location, property_price FROM property WHERE id = $1`,
      [id]
    );
    ...
    const { city, location, property_price } = current.rows[0];
    const minPrice = property_price * 0.8;
    const maxPrice = property_price * 1.2;

    let result = await pool.query(
      `SELECT id, property_name, city, location, property_price, property_bhk, property_type_id
       FROM property
       WHERE location = $1 AND property_price BETWEEN $2 AND $3 AND id != $4
       ORDER BY created_at DESC
       LIMIT 4`,
      [location, minPrice, maxPrice, id]
    );
    ...
    if (result.rows.length < 4) {
      result = await pool.query(
        `SELECT id, property_name, city, location, property_price, property_bhk, property_type_id
         FROM property
         WHERE city = $1 AND property_price BETWEEN $2 AND $3 AND id != $4
         ORDER BY created_at DESC
         LIMIT 4`,
        [city, minPrice, maxPrice, id]
      );
    }
